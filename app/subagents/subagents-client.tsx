...
export default function SubagentsClient({ subagents }: SubagentsClientProps) {
  const [search, setSearch] = useState("")
  const [activeCategory, setActiveCategory] = useState("all")

  const categories = useMemo(() => {
    const cats = new Set(subagents.map(s => s.category))
  ...
  }, [subagents, search, activeCategory])

  return (
    <div className="min-h-screen">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="mb-10">
          <h1 className="text-display-2 mb-3">Subagents</h1>
          <p className="text-muted-foreground">{subagents.length} specialized subagents</p>
        </div>
